...
umaFuncao : FUNCT tipoOuVoid IDENT   {  TS_entry nodo = ts.pesquisa($3);
                            if (nodo != null) 
                              yyerror("(sem) nome >" + $3 + "< jah declarada");
                          else ts.insert(new TS_entry($3, (TS_entry)$2, ClasseID.NomeFuncao)); 
                        }
...
ListaParametros : ListaParametros ',' type IDENT 
                | type IDENT 
                ;
...
nomeId : IDENT         {  TS_entry nodo = ts.pesquisa($1);
                          ...
                          else ts.insert(new TS_entry($1, currType, currClass)); 
                        }
        ;
